...
def callback(msg):
        global yaw, pitch  ,roll, theta, x, y
        ...
        if len(l) == 2:
                ...
                pitch = acos(1/ (1+abs(x/10)))
                roll  = acos(1/ (1+abs(y/10)))
                yaw   -= theta
                l.pop(0)
                pub_icp.publish(44.3 + degrees(yaw))
                
                pub_icp_tx.publish(degrees(pitch))
                pub_icp_ty.publish(degrees(roll))
...
def callback2(msg):
        ...
        q = quaternion_from_euler(roll, pitch, yaw+0.77)
